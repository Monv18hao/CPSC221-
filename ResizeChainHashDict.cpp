...
void ResizeChainHashDict::rehash() {
  ...
  for (int i=0; i<pSize; i++) {
    ChainNode *r = oldTable[i];
    while (r != NULL) {
      ChainNode *k = table[hash(r->keyID)];
      ChainNode *temp = new ChainNode();
      temp->key = r->key;
      temp->keyID = r->keyID;
      temp->data = r->data;
      temp->next = k;
      table[hash(r->keyID)%size] = temp;
      r = r->next;
    }
  }
  delete [] oldTable;
...
bool ResizeChainHashDict::find(PuzzleState *key, PuzzleState *&pred) {
  ...
  string uID = key->getUniqId();
  ChainNode *row = table[hash(uID)];
  return find_helper(row, uID, pred);
}

bool ResizeChainHashDict::find_helper(ChainNode *r, string keyID, PuzzleState *&pred) {
  int count = 0;
  while (r != NULL) {
    if (keyID == r->keyID) {
      pred = r->data;
